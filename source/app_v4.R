                                                     ...
                                                     tabPanel("DGE Analysis: DEGseq",
                                                                ...
                                                                fluidRow(
                                                                  ...
                                                                  column(6,
                                                                         actionButton(inputId = "run_DEGexp",
                                                                                      label = "RUN DEGexp"),
                                                                         p("Please wait for the result tables below show up..."))),
                                                              ...
                                                              wellPanel(
                                                                ...
                                                                fluidRow(
                                                                  column(6,
                                                                         wellPanel(
                                                                           plotOutput(outputId = "output_up.dn_dn.up_heatmap")
                                                                         ))),
                                                                fluidRow(
                                                                  column(6,
                                                                         wellPanel(DT::dataTableOutput(outputId = "output_up.dn_dn.up_table")))
                                                                  ))),
                                                     
                                                     
                                                     ## --------------- DEseq2 ----------------
                                                     tabPanel("DGE Analysis: DEseq2",
                                                              wellPanel(
                                                                tags$h3("Data Preparation"),
                                                                fluidRow(
                                                                  column(4,
                                                                         selectInput(inputId = "rna_deseq2_sample_choices",
                                                                ...
                                                                fluidRow(
                                                                  column(4,
                                                                         selectizeInput(inputId = "rna_deseq2_groupby_choices",
                                                                                        label = "Group by:",
                                                                                        choices = NULL,
                                                                                        multiple = TRUE,
                                                                                        selected = NULL)),
                                                                  column(2,
                                                                         radioButtons(inputId = "isSingleEnd",
                                                                                      label = "Select Single or Pair End",
                                                                                      choices = list("With Interaction" = "true", 
                                                                                                     "Without Interaction" = "false"),
                                                                                      selected = "true")),
                                                                  column(3,
                                                                         actionButton(inputId = "generate_deseqdt",
                                                                                      label = "Generate DESeqDataSet"))
                                                                ),
                                                                ...
                                                                fluidRow(
                                                                  column(6,
                                                                         ...
                                                                         verbatimTextOutput("display_deseqdt")),
                                                                  column(6,
                                                                         tags$h4("Trimmed DESeqDataSet Summary:"),
                                                                         verbatimTextOutput("display_trim_result_deseq2"),
                                                                         verbatimTextOutput("display_trimmed_deseqdt"))))))))))
...
server <- function(input, output, session) {
  ...
  ## ------------  Read in Data and update choices -------------
  ...
  ## read in and display count table and update sample choice
  observeEvent(input$readin_rna_count,{
    ...
    if(nrow(parseFilePaths(roots = volumes,
      ...
      updateSelectInput(session, 
                        inputId = "rna_deseq2_sample_choices", 
                        label = "Select samples:", 
                        choices = colnames(rv$ct)[7: ncol(rv$ct)], 
                        selected = NULL)
    }
  ...
  # read in info table and update group-by choice
  observeEvent(input$readin_rna_info,{
    # process if both count and info table are selected and count table are read in
    if(nrow(parseFilePaths(roots = volumes,
                           selection = input$rna_info_dir)) != 0
       & nrow(parseFilePaths(roots = volumes,
                             selection = input$rna_count_dir)) != 0
       # & !is.null(output$display_rna_count)
       ){
      ...
      updateSelectizeInput(session, 
                        ...
                        selected = NULL)
      updateSelectizeInput(session, 
                           inputId = "rna_deseq2_groupby_choices", 
                           label = "Group by:", 
                           choices = colnames(rv$info)[2: ncol(rv$info)], 
                           selected = NULL)
    }
  ...
  observeEvent(input$generate_up.dn_dn.up_table_and_heatmap,{
    ...
    # merge to create the up.dn_gn.up table
    ...
    output$display_venn_diagram1 <- renderPlot({
      draw.pairwise.venn(area1 = length(rv$trt2_trt1_up),
    ...
    output$display_venn_diagram2 <- renderPlot({
      draw.pairwise.venn(area1 = length(rv$trt2_trt1_dn),
    ...
    output$output_up.dn_dn.up_table = DT::renderDataTable({
    ...
    output$output_up.dn_dn.up_heatmap <- renderPlot({
      print(rv$p3)
    })
    output$test1 <- renderPrint({head(rv$up.dn_dn.up_table_long)})
    output$test2 <- renderPrint({summary(rv$up.dn_dn.up_table_long)})
      
  })
  
  
  # export up.dn_dn.up_table, venn diagram, two-circle heatmap
  observeEvent(input$export_change_in_gene_exp,{
    ...
    output$display_change_in_gene_exp_dir <- renderPrint({
      cat(paste("table export to:", file_path, "plot export to:", file_path1, sep = "\n"))
    })
  ...
  ## ---------------- DEseq2 -----------------------
  observeEvent(input$generate_deseqdt,{
    if( !is.null(input$rna_deseq2_sample_choices)){
      dt <- as.matrix(rv$ct[ ,input$rna_deseq2_sample_choices])
      row.names(dt) <- rv$ct$gene
      rv$cts <- dt

                     ...
                     fluidRow(
                       column(6,
                              wellPanel(
                                ...
                                fluidRow(
                                  plotOutput("display_heatmap_expl")
                                  # imageOutput(outputId = "display_heatmap_expl")
                                ))),
                       column(6,
                              wellPanel(
                                tags$h3("PCA Plot"),
                       ...
                       column(6,
                              wellPanel(
                                textInput(inputId = "ma2_title",
                                          ...
                                          value = ""),
                                plotOutput(outputId = "ma2_degseq")))),
                     fluidRow(
                       column(6,
                     ...
                     fluidRow(
                       column(3,
                              selectInput(inputId = "dna_nc",
                                          label = "Select columns for negative control",
                                          choices = NULL,
                                          multiple = TRUE,
                                          selected = NULL)),
                       column(3,
                              selectInput(inputId = "dna_pc",
                                          label = "Select columns for positive control",
                                          choices = NULL,
                                          multiple = TRUE,
                                          selected = NULL)),
                       column(4,
                              selectInput(inputId = "dna_trt",
                                          label = "Select columns for treatment of interest",
                                          choices = NULL,
                                          multiple = TRUE,
                                          selected = NULL))
                     ),
                     fluidRow(
                       column(3,
                              textInput(inputId = "dna_three_sample_name",
                                        label = "Enter sample name for three treatments selected above, seperated by comma",
                                        value = "trt1, trt2, trt3"))
                     ),
                     fluidRow(
                       column(3,
                              selectInput(inputId = "dna_reg",
                                          label = "Select one region of interest",
                                          choices = c("Promoter", "5' UTR", "Body", "3' UTR", "Downstream"),
                                          multiple = FALSE,
                                          selected = "Promoter")),
                       column(4,
                              textInput(inputId = "methyl%_top_n",
                                        label = "Enter number to display top n methyl% genes",
                                        value = "20")),
                       column(3,
                              actionButton(inputId = "generate_methyl_DE_result",
                                           label = "Generate Result"))
                     ),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$generate_result_expl,{
    ...
    withProgress(message = "Generating Result",
                 ...
                 expr = {
                   ...
                   rv$sample_name_list_expl <- rv$info$`Sample Name`[match(input$sample_label_selected_expl, rv$info$`Sample Label`)]
                   rv$ct_expl <- rv$ct[, c(input$gene_col_selected_expl, rv$sample_name_list_expl)]
                   colnames(rv$ct_expl) <- c("gene", input$sample_label_selected_expl)
                   
                   rv$ct_expl_log2 <- rv$ct_expl
                   rv$ct_expl_log2[, 2:ncol(rv$ct_expl_log2)] <- log2(rv$ct_expl_log2[, 2:ncol(rv$ct_expl_log2)] +1)
                   
                   rv$ct_expl_log2_long <- melt(rv$ct_expl_log2,
                                                id.vars = 1, # "gene"
                                                measure.vars = 2:ncol(rv$ct_expl_log2),
                                                variable.name = "sample",
                                                value.name = "count")
                   
                   rv$info_expl <- rv$info[match(input$sample_label_selected_expl, rv$info$`Sample Label`), c("Sample Label", input$covariate_selected_expl)]
                   colnames(rv$info_expl) <- c("sample", input$covariate_selected_expl)
                   rv$info_expl[,2] <- as.factor(rv$info_expl[,2])
                   
                   rv$ct_info_expl_log2_long <- merge(rv$ct_expl_log2_long, rv$info_expl, by = "sample")
                   
                   # histogram
                   incProgress(0.2, detail = "Ploting Histogram")
                   rv$count_hist <- ggplot(data = rv$ct_info_expl_log2_long,
                                           aes(x = count)) +
                     xlab("") +
                   ...
                   # boxplot
                   incProgress(0.2, detail = "Ploting Boxplot")
                   rv$boxplot <- ggplot(data = rv$ct_info_expl_log2_long,
                                        aes(x = sample,
                                            y = count,
                                            # fill = input$covariate_selected_expl
                                            fill = eval(parse(text = input$covariate_selected_expl ))
                                        )) + 
                     geom_boxplot() +
                     xlab("") +
                     ylab(expression(log[2](count + 1))) + 
                   ...
                   # heatmap s2s 
                   incProgress(0.2, detail = "Ploting Heatmap")
                   sample_dist <- dist(t(as.matrix(rv$ct_expl_log2)[,-1]))
                   rv$sample_dist_matrix <- as.matrix(sample_dist)
                   rv$heatmap <- pheatmap(rv$sample_dist_matrix,
                                          col = colorRampPalette( rev(brewer.pal(9, "Blues")) )(255))
                   output$display_heatmap_expl <- renderPlot({
                     print(rv$heatmap)
                   })
                   
                   # pca
                   incProgress(0.2, detail = "Ploting PCA plot")
                   matrix_pca <- t(rv$ct_expl_log2[, -1])
                   ...
                   df_out$sample <- colnames(rv$ct_expl_log2)[-1]
                   df_out <- merge(df_out, rv$info_expl, by = "sample")
                   ...
                   incProgress(0.1, detail = "Processing 100%")
                   Sys.sleep(1)
                 })
  ...
  observeEvent(input$trim,{
    if(length(input$sample_label_selected_degseq) != 3){
      ...
      rv$sample_name_list_degseq <- rv$info$`Sample Name`[match(input$sample_label_selected_degseq, rv$info$`Sample Label`)]
      rv$ct_degseq <- rv$ct[, c(input$gene_col_selected_degseq, rv$sample_name_list_degseq)]
      colnames(rv$ct_degseq) <- c("gene", input$sample_label_selected_degseq)
      
      rv$ct_degseq <- subset(rv$ct_degseq,
                             rowSums(rv$ct_degseq[, -1]) > as.numeric(input$row_sum))
      ...
      output$trimmed_left <- renderPrint({
        cat(
          paste(nrow(rv$ct_degseq),
                ...
                "genes",
                sep = " "))})
      output$display_trimmed_ct <- renderPrint({
        summary(rv$ct_degseq)
      })                         
    }  
  })
  ...
  observeEvent(input$run_DEGexp,{
    ...
    output$ma1_degseq <- renderPlot({})
    output$ma2_degseq <- renderPlot({})
    ...
    withProgress(message = "Running DEGexp",
                 ...
                 expr = {
                   ...
                   rv$ma1_degseq <- ggplot(rv$table_trt2_trt1,
                                           aes(x = mu,
                   ...
                   rv$ma2_degseq <- ggplot(rv$table_trt3_trt2,
                                           aes(x = mu,
                                               y = `log2(Fold_change)`,
                                               colour = clr,
                                               shape = pch)) +
    ...
    if(!is.null(rv$table_trt2_trt1)){
      withProgress(message = "Generating results",
                   ...
                   expr = {
                     ...
                     rv$merge1  <- rv$table_trt2_trt1[rv$table_trt2_trt1$GeneNames %in% rv$up.dn_dn.up_genes, c("GeneNames","log2(Fold_change) normalized")]
                     rv$merge2  <- rv$table_trt3_trt2[rv$table_trt3_trt2$GeneNames %in% rv$up.dn_dn.up_genes, c("GeneNames","log2(Fold_change) normalized")]
                     
                     rv$up.dn_dn.up_table <- merge(rv$merge1, rv$merge2, by = "GeneNames")
                     colnames(rv$up.dn_dn.up_table) <- c("gene",paste(input$sample_label_selected_degseq[2],"-",input$sample_label_selected_degseq[1]), paste(input$sample_label_selected_degseq[3],"-",input$sample_label_selected_degseq[2]))
                     rv$up.dn_dn.up_table <- rv$up.dn_dn.up_table[order(rv$up.dn_dn.up_table[ ,2], decreasing = TRUE), ]
                     
                     
                     # venn diagram and sig# table
                     ...
                     output$change_number <- renderTable({
                       ...
                       col2 <- c(length(rv$trt2_trt1_up), length(rv$trt2_trt1_dn))
                       ...
                       col4 <- c(length(rv$trt3_trt2_dn), length(rv$trt3_trt2_up))
                       col5 <- c("intersection", "intersection")
                       col6 <- c(length(rv$up.dn),length(rv$dn.up))
                     ...
                     rv$up.dn_dn.up_table_long$Comparison <- factor(rv$up.dn_dn.up_table_long$Comparison,
                                                                    # inner circle trt3-trt1 and outer circle trt2-trt1
                                                                    levels = c(colnames(rv$up.dn_dn.up_table)[3],
                                                                               colnames(rv$up.dn_dn.up_table)[2]))
                     rv$lvls <- rv$up.dn_dn.up_table_long[rv$up.dn_dn.up_table_long$Comparison == colnames(rv$up.dn_dn.up_table)[2], ]
                     ...
                     rv$two_circle_heatmap <- ggplot(data = rv$up.dn_dn.up_table_long) +                        
                       coord_polar("y",
                                   start = 0,
                                   direction = -1) +
                       geom_tile(aes(x = as.numeric(Comparison),     
                                 ...
                                 color = "white") +                   
                       geom_text(data = rv$up.dn_dn.up_table_long[Comparison == colnames(rv$up.dn_dn.up_table)[2], ],  
                                 aes(x = rep(1.75,
                                             nlevels(gene)),
                                     y = gene,
                                     label = unique(gene),
    ...
    if(!is.null(rv$dds_res)){
      withProgress(message = "Extracting results with contrast", 
                   ...
                   expr = {
                     ...
                     # rv$deseq2_dsp_est <- print(plotDispEsts(rv$dds_res))
                     output$deseq2_estimated_dispersion <- renderPlot({
                       rv$deseq2_dsp_est <- plotDispEsts(rv$dds_res)
                       print(rv$deseq2_dsp_est)
                     })
  ...
  ## ----------- DNA -------------
  ## --------- Annotate and update selectinput------------
  ...
  observeEvent(input$generate_expl_dna,{
    ...
    if(class(rv$dt_dna2) == "try-error"){
      ...
      output$test_dna <- renderPrint( paste0("Removed ",
  ...
  observeEvent(input$generate_methyl_DE_result, {
    ...
    #                   silent = TRUE)
    # 
    # if(class(rv$dt_dna3) == "try-error"){
    #   showNotification(rv$dds[1],
    #                    type = "error",
    #                    duration = 15)
    # }else{
    #   output$dna_trim_result2 <- renderPrint( paste0("Removed ",
    #                                          nrow(rv$dt_dna) - nrow(rv$dt_dna1),
    #                                          " genes with unmapped region and ",
    #                                          nrow(rv$dt_dna1) - nrow(rv$dt_dna3),
    #                                          " genes with all NA value, with",
    #                                          nrow(rv$dt_dna3), 
    #                                          " genes left."))
    #   rv$dt_dna3$reg <- as.character(rv$dt_dna3$anno)
    #   
    #   rv$dt_dna3$reg[substr(rv$dt_dna3$anno,
    #                         1,
    #                         8) == "Promoter"] <- "Promoter"
    #   rv$dt_dna3$reg[substr(rv$dt_dna3$anno,
    #                         1,
    ...
    #                         4) %in% c("Dist",
    #                                   "Down")] <- "Downstream"
    #   rv$dt_dna3$reg <- factor(rv$dt_dna3$reg,
    #                            levels = c("Promoter",
    #                                       "5' UTR",
    #                                       "Body",
    #                                       "3' UTR",
    ...
    # 
    # 
    # dtN <- as.matrix(rv$dt_dna3[, c(input$dna_nc[1], input$dna_pc[1], input$dna_trt[1])])
    # dtX <- as.matrix(rv$dt_dna3[, c(input$dna_nc[2], input$dna_pc[2], input$dna_trt[2])])
    # dtX <- apply(dtX,
    #              2,
    #              function(a) {
    #                a[is.na(a)] <- a[a == 0] <- 0.5
    ...
    # colnames(pct) <- unlist(strsplit(input$dna_three_sample_name, ","))
